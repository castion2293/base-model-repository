...
namespace Pharaoh\BaseModelRepository\Helpers;
...
class SqlFormatterSupport
{
    ...
    protected static function init()
    {
        ...
        self::$regex_reserved_toplevel = str_replace(
            ' ',
            '\\s+',
            '(' . implode('|', array_map(array(__CLASS__, 'quote_regex'), self::$reserved_toplevel)) . ')'
        );
        self::$regex_reserved_newline = str_replace(
            ' ',
            '\\s+',
    ...
    protected static function getNextToken($string, $previous = null)
    {
        ...
        if (preg_match(
            '/^([0-9]+(\.[0-9]+)?|0x[0-9a-fA-F]+|0b[01]+)($|\s|"\'`|' . self::$regex_boundaries . ')/',
            $string,
            $matches
        )) {
            return array(
        ...
        if (!$previous || !isset($previous[self::TOKEN_VALUE]) || $previous[self::TOKEN_VALUE] !== '.') {
            ...
            if (preg_match(
                '/^(' . self::$regex_reserved_toplevel . ')($|\s|' . self::$regex_boundaries . ')/',
                $upper,
                $matches
            )) {
                return array(
            ...
            if (preg_match(
                '/^(' . self::$regex_reserved_newline . ')($|\s|' . self::$regex_boundaries . ')/',
                $upper,
                $matches
            )) {
                return array(
            ...
            // Other Reserved Word
            if (preg_match(
                '/^(' . self::$regex_reserved . ')($|\s|' . self::$regex_boundaries . ')/',
                $upper,
                $matches
            )) {
                return array(
    ...
    public static function format($string, $highlight = true)
    {
        ...
        foreach ($tokens as $i => $token) {
            ...
            elseif ($token[self::TOKEN_TYPE] === self::TOKEN_TYPE_RESERVED_TOPLEVEL) {
                ...
                if (strpos($token[self::TOKEN_VALUE], ' ') !== false || strpos(
